...
func main() {
	...
	case "":
		if runActorLoop(cfg, logPath, reg, port, guiDir, savePath, mcpPipe) {
			os.Exit(1)
		}
...
func runActorLoop(cfg common.Config, logPath string, reg *tools.Reg, port string, guiDir string, savePath string, mcpPipe bool) (vendorFailed bool) {
	host := newCLIHost()
	j := jobs.NewJobs(host)
