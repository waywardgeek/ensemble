...
func main() {
	...
	case "":
		if runActorLoop(cfg, logPath, reg, port, guiDir, savePath) {
			os.Exit(1)
		}
...
func runActorLoop(cfg common.Config, logPath string, reg *tools.Reg, port string, guiDir string, savePath string) (vendorFailed bool) {
	host := newCLIHost()
	j := jobs.NewJobs(host)
